...
var index = require('./index-5862fa85.js');
...
class Body {
	constructor(body, {
		...
		} else if (body instanceof Stream) ; else if (body instanceof index.FormData) {
			// Body is FormData
			body = index.formDataToBlob(body);
	...
	async formData() {
		...
		const {toFormData} = await Promise.resolve().then(function () { return require('./multipart-parser-07752608.js'); });
...
const extractContentType = (body, request) => {
	...
	if (body instanceof index.FormData) {
...
const apiHandler = async (url, init) => {
